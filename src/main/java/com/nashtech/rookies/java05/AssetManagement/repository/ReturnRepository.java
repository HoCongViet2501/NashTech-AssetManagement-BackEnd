...
public interface ReturnRepository extends JpaRepository<Returning, Long> {
    @Query(value = "SELECT r.id, r.is_delete, r.returned_date , r.state , r.accepted_by , r.assignment_id , r.request_by from information i inner join returnings r on r.request_by = i.user_id  where i.location = :location and r.is_delete = false ", nativeQuery = true)
